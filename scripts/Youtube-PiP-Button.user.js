...
(function () {
    ...
    // PiP Fix
    const pip_fix = v => v.addEventListener('webkitpresentationmodechanged', e => e.stopPropagation(), true);
    let v = document.querySelector('video');
    if (v) pip_fix(v);

    // Create PiP Button
    let pip_button = document.createElement("button");
        ...
        mutationList.forEach((mutation) => {
            if (mutation.type == 'childList') {
                mutation.addedNodes.forEach(node => {
                    ...
                    if (node.nodeName == 'VIDEO') { // PiP Fix
                        pip_fix(node);
                    }
